...
function doSort(data, columnsById, sortOrder, renderContext) {
    return [...data].sort((a, b) => {
        for (const column of sortOrder) {
            const columnSchema = schemaDetectorsByType.get(columnsById.get(column.id).schema);

            const aValue = a.get(column.id);
            const bValue = b.get(column.id);

            const ret = columnSchema.comparator(aValue, bValue, column.direction, renderContext);
...
function DataTable({columns, dataRows, renderContext, initialSort=[]}) {
    ...
    const [sortedData, setSortedData] = useState(doSort(dataRows, columnsById, initialSort, renderContext));

    const [sortingColumns, setSortingColumns] = useState(initialSort);

    const setSorting = (newSortingColumns) => {
        const newSortedData = doSort(sortedData, columnsById, newSortingColumns, renderContext);
    ...
    const renderCellValue = useMemo(() => {
        return ({rowIndex, columnId, isDetails}) => {
            ...
            if (sortedData[rowIndex].has(columnId)) {
                ...
                const schemaHasRenderer = hasSchema ? schemaDetectorsByType.get(columnsById.get(columnId).schema).hasOwnProperty("renderer") : false;
                const schemaHasDetailsRenderer = hasSchema ? schemaDetectorsByType.get(columnsById.get(columnId).schema).hasOwnProperty("detailsRenderer") : false;

                if (isDetails && hasSchema && schemaHasDetailsRenderer) {
                    return schemaDetectorsByType.get(columnsById.get(columnId).schema).detailsRenderer(cellValue, renderContext);
                } else if (hasSchema && schemaHasRenderer) {
                    return schemaDetectorsByType.get(columnsById.get(columnId).schema).renderer(cellValue, renderContext);
                } else {
                    return cellValue;
                }
            } else {
