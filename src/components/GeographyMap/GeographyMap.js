...
function project(inputCoordinates) {
    const [longitude, latitude] = proj4("EPSG:32015", "EPSG:4326", inputCoordinates);
...
function GeographyMap({rtif: theRtif}) {
    ...
    return <EuiPageTemplate
        ...
        <EuiFlexGroup direction="column" gutterSize={"none"}>
            <EuiFlexItem>
                <MapContainer center={[40.754672, -73.986754]} zoom={13} scrollWheelZoom={true} style={{"height": "100%"}}>
                    <TileLayer
                    ...
                    {geographies
                        .filter(geography => geographyHasStatePlaneCoordinates(geography) || geographyHasGeodeticCoordinates(geography))
                        .map(geography => {

                            const position = geographyHasGeodeticCoordinates(geography)
                                ? [geography.get("latitude"), geography.get("longitude")]
                                : project([geography.get("easting"), geography.get("northing")]);

                            return <Marker position={position} key={geography.get("locationName")}>
                                <Popup>
