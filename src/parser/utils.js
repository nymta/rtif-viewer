...
export function formatTripName(trip, timetable) {
  ...
  const originTerminal = formatLocationAbbreviation(
    trip.get("originLocation"),
    trip.get("tripType")
  );
  const destinationTerminal = formatLocationAbbreviation(
    trip.get("destinationLocation"),
    trip.get("tripType")
  );
...
export function formatTime(rtifTime) {
  if (!!rtifTime) {
    if (rtifTime >= 0) {
      ...
      const offsetTime =
        DAY_IN_CENTIMINUTES - (Math.abs(rtifTime) % DAY_IN_CENTIMINUTES);
